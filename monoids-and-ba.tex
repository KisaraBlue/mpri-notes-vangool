...
The above expression is called star-free because it can be built from  single letters and the set $A^*$ of all words using only the operations of concatenation, union (that we denote by $\cup$ instead of $+$) and complementation, $()^c$.% The name stems from the fact that the operation $()^*$ is essentially not used in this expression, noting that $A^*$ can be written as $\emptyset^c$.
%
...
\end{definition}
\begin{exercise}\easy
  % An element in a monoid $M$ is called \emph{invertible} if it has a two-sided inverse.
...
Prove that $S$ divides $T$ if, and only if, there exist a semigroup $U$, an injective homomorphism $i \colon U \to T$, and a surjective homomorphism $g \colon U \to S$. %({\it Hint.} Use Proposition~\ref{prop:homthm}.)
\end{exercise}
\begin{proposition}
...
% 
  %$\theta$ a congruence on $T'$, and $f \colon T'/{\theta} \to S(L)$ an isomorphism. For any $a \in A$, choose an element $g(a) \in T'$ such that $f(g(a)) = [a]_{\sim_L}$. Denote by $\bar{g} \colon A^+ \to T'$ the unique homomorphism extending $g$, by the existence part of Proposition~\ref{prop:free}. Then $f \circ \bar{g}$ is a homomorphism $A^+ \to S(L)$ with $f(\bar{g}(a)) = [a]_{\sim_L}$ for every $a \in A$, so $f \circ p_\theta \circ \bar{g} = p_{\sim_L}$, using the uniqueness part of Proposition~\ref{prop:free}. In particular,%Thus, $p_\theta \circ \bar{g} = f^{-1} \circ p_{\sim_L}$, and $\bar{g}$ recognizes $L$.
\end{proof}
...
% (Hint: the infimum of $a$ and $b$ is $a \cdot b$, and the supremum of $a$ and $b$ is $a + b - ab$.)
 every element of $B$ has a complement with respect to $\leq$. \emph{Hint:} first show that $a + a = 0$ for all $a \in B$.
% (Hint: the complement is 1 - a)
...
\begin{theorem}[Discrete duality for semigroups, quotients and subs]\label{thm:semigroup-quotient-duality}
  Let $S$ be a semigroup and $(\Po(S), \cdot, \backslash, \slash)$ its discrete dual residuated Boolean algebra. The congruence lattice of $S$ is dually isomorphic to the lattice of complete Boolean residuation ideals of $\Po(S)$.%Under discrete duality, monoid quotients of $M$ correspond to complete Boolean residuation ideals of $\Po(M)$.
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  %there exist states $q_0, q_1, q_2 \in Q$ such that $q_0 \in I$, $q_2 \in F$, $q_0 \stackrel{x}{\to} q_1$, and $q_1 \stackrel{w}{\to} q_2$. Equivalently, $w$ is accepted by $\cA' = (Q,A,\delta,\blacklozenge_x(I),F)$. This proves the first equality, the proof of the second equality is very similar. 
\end{proof}


\begin{proposition}\label{prop:residuationideal}
For any automaton $\cA$, $B(\cA)$ is a finite (hence complete) Boolean residuation ideal in $\Po(A^+)$. %For any $K \in \Po(A^*)$, %the following hold:
%\begin{enumerate}
%\item if $L \in R(\cA)$, then $K \backslash L \in \cL(\cA)$ and $L \slash K \in \cL(\cA)$;
%\item \label{it:latticeresideal} 
%if $L \in \cL(\cA)$, then $K \backslash L \in \cL(\cA)$ and $L \slash K \in \cL(\cA)$ and if $L \in \cB(\cA)$, then $K \backslash L \in \cB(\cA)$ and $L \slash K \in \cB(\cA)$.
%;
%\item \label{it:BAresideal} if $L \in \cB(\cA)$, then $K \backslash L \in \cB(\cA)$ and $L \slash K \in \cB(\cA)$.
%\end{enumerate}
\end{proposition}
\begin{proof}
